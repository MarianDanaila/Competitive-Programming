            ...
            for j in range(m):
                if j == 0:
                    ...
                    if matrix[i][j] == 0:
                        ...
                        dp[i][j] = dp[i][j-1]+1  # count consectuive left zeroes
        ...
        for j in range(m):  # from left to right
            for i in range(n):  # from top to bottom
                ...
                for k in range(i, -1, -1):  # from i to top(count the number of rectangles that can be made)
                    minimum = min(minimum, dp[k][j])  # the number of rectangles that can be made with the right edge
                    # extended from mat[i][j] to mat[k][j]
                    if minimum == 0:
                        break
                    result += minimum
        return result
